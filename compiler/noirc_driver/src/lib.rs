...
impl CompileOptions {
    pub fn as_ssa_options(&self, package_build_path: PathBuf) -> SsaEvaluatorOptions {
        SsaEvaluatorOptions {
            ...
            // Debug instrumentation (`nargo debug`, `nargo trace`) needs the SSA
            // pipeline to preserve the source-line mapping, so it runs the reduced
            // `OptimizationLevel::Debug` pass list. Every caller of this function
            // previously threaded `self.instrument_debug` in by hand; deriving it
            // here keeps the upstream signature and cannot get out of step.
            optimization_level: if self.instrument_debug {
