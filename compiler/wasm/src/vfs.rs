...
use nargo::parse_all;
use noirc_driver::{
    CompileOptions, add_dep, file_manager_with_stdlib, link_to_debug_crate, prepare_crate,
    prepare_dependency,
};
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            VfsError::BadPackageName { name, .. } => match name {
                Some(name) => {
                    write!(f, "`[package].name` is `{name}`, which is not a valid Noir crate name")?
                }
            ...
            VfsError::MissingEntry { entry, was_declared, .. } => {
                if *was_declared {
                    ...
                    write!(f, "the crate root `{entry}` is not in the virtual filesystem")?;
                }
            }
...
pub fn resolve_vfs(
    ...
    let keyed: BTreeMap<String, &String> = files.iter().map(|(p, s)| (vfs_key(p), s)).collect();
    ...
    while !queue.is_empty() {
        ...
        let manifest: RawManifest =
            toml::from_str(manifest_text).map_err(|err| VfsError::ManifestNotToml {
...
/// The directory whose files the source-level debug instrumenter rewrites.
///
/// Native `nargo` instruments every parsed file having the entry package's
/// `src/` as an ancestor and no others — `nargo::ops::debug::instrument_package_files`
/// takes `package.entry_path.parent()` and compares ancestors. A dependency is
/// deliberately NOT instrumented: `__debug_*` calls in a library would be recorded
/// against a crate the user is not stepping through.
///
/// This is the VFS spelling of that same rule, kept as its own function so the
/// correspondence is one place rather than inline in a builder.
fn instrumentation_root(plan: &ResolvedProgram) -> Option<String> {
    let entry = Path::new(&plan.entry_point);
    entry.parent().map(|p| vfs_key(p))
}
...
///
/// `for_debugging` selects the **debugging compile path**, and the distinction is not
/// cosmetic: the source-level instrumenter runs only there, so an artifact compiled
/// without it carries no `__debug_*` calls and a tracer consuming it records **one
/// event and no steps**. That was measured, not reasoned about — the first end-to-end
/// run of "compile in wasm, then trace in wasm" reported `ok` from both modules and
/// produced an empty trace, which is exactly the shape of failure that passes every
/// obvious assertion. `mode: "debug"` exists so that cannot happen quietly.
pub fn context_for(
    ...
    // Instrument BEFORE the `Context` is built, because the instrumenter rewrites the
    // parsed ASTs the context will own. `nargo::ops::debug::prepare_package_for_debug`
    // has the same order for the same reason, and reversing it silently produces an
    // uninstrumented program.
    let debug_instrumenter = if for_debugging {
        let root = instrumentation_root(plan);
        let mut instrumenter = DebugInstrumenter::default();
        for (file_id, parsed_file) in parsed_files.iter_mut() {
            let Some(path) = file_manager.path(*file_id) else { continue };
            let key = vfs_key(path);
            if let Some(root) = root.as_ref() {
                if key == *root || key.starts_with(&format!("{root}/")) {
                    instrumenter.instrument_module(&mut parsed_file.0);
                }
            }
        }
        Some(instrumenter)
    ...
    if let Some(instrumenter) = debug_instrumenter {
        // After `prepare_crate` and before the dependency edges below, matching
        // `prepare_package_for_debug`. `link_to_debug_crate` registers `__debug/lib.nr`
        // from the stdlib file manager and makes the root crate depend on it, which is
        // what gives the instrumented calls something to resolve to.
        link_to_debug_crate(&mut context, root_id);
        context.debug_instrumenter = instrumenter;
    }
...
/// Compile options for the debugging path, mirroring
/// `nargo::ops::debug::compile_options_for_debugging(acir_mode = false,
/// skip_instrumentation = false, ..)` — which is exactly what `nargo trace` passes.
///
/// `force_brillig` is load-bearing rather than a performance choice: the tracer steps
/// unconstrained bytecode, so an ACIR-mode compile of the same program yields a trace
...
///
/// `for_debugging` selects the instrumented path — see `context_for`. A caller that
/// wants an artifact a tracer can consume must pass `true`; one that only wants the
/// circuit must not, since instrumentation changes the program.
///
...
pub fn compile_resolved(
    ...
    let (mut context, root_id) = context_for(plan, files, for_debugging);
    let options =
        if for_debugging { debugging_compile_options() } else { CompileOptions::default() };

    if as_contract {
        match noirc_driver::compile_contract(&mut context, root_id, &options) {
            Ok((contract, warnings)) => {
                // No `nargo::ops::optimize_contract` here, and its absence is the
                // point rather than an omission: upstream #13085 ("ACIR
                // instrumentation and optimization") moved ACIR optimization INTO
                // the compiler — `noirc_evaluator::ssa` now calls
                // `acvm::compiler::optimize` as it generates — and deleted
                // `tooling/nargo/src/ops/optimize.rs` in the same commit. A
                // compiled contract arrives already optimized. This is exactly
                // what that commit did to `compile.rs` and `compile_new.rs`, the
                // two siblings of this file, which is why it is the right port
                // rather than a way to make the build pass.
        ...
        match noirc_driver::compile_main(&mut context, root_id, &options, None) {
            Ok((program, warnings)) => {
                // See the contract arm: optimization is the compiler's since
                // upstream #13085.
                let positioned = position_diagnostics(
                    &warnings.into_iter().collect::<Vec<_>>(),
                    &context.file_manager,
...
mod tests {
    ...
    fn three_file_tree_with_dep() -> BTreeMap<PathBuf, String> {
        tree(&[
            ...
            ("app/src/main.nr", "fn main(x: Field) -> pub Field {\n    util::twice(x)\n}\n"),
    ...
    fn a_three_file_tree_with_a_local_dependency_compiles() {
        let files = three_file_tree_with_dep();
        let plan = resolve_vfs(&files, "app").expect("the tree resolves");
        let compiled = compile_resolved(&plan, &files, false, false);
        match compiled {
    ...
    fn the_refusals_line_moves_when_the_dependency_moves() {
        let manifest_top = "[package]\nname = \"app\"\ntype = \"bin\"\n\n[dependencies]\ndep = { git = \"https://example.com/x\", tag = \"v1\" }\n";
        let manifest_low = "[package]\nname = \"app\"\ntype = \"bin\"\n\n\n\n\n[dependencies]\ndep = { git = \"https://example.com/x\", tag = \"v1\" }\n";
    ...
    fn a_multi_file_single_crate_tree_resolves_every_module() {
        ...
        assert_eq!(
            plan.sources,
            vec!["app/src/main.nr".to_string(), "app/src/util.nr".to_string()]
        );
    ...
    fn a_type_error_reports_the_right_file_line_and_column() {
        ...
        let plan = resolve_vfs(&files, "app").expect("resolves");
        let diagnostics =
            compile_resolved(&plan, &files, false, false).err().expect("a type error");
        assert!(!diagnostics.is_empty(), "at least one diagnostic");

        let in_util: Vec<_> = diagnostics.iter().filter(|d| d.file == "app/src/util.nr").collect();
        assert!(
            !in_util.is_empty(),
    ...
    /// The debugging compile produces the source-level tables a tracer needs, and the
    /// ordinary compile does not.
    ///
    /// **This test exists because of a measured failure, not a hypothesis.** Compiling
    /// a package from a VFS inside `noir_wasm` and then tracing the result inside
    /// `noir_tracer_wasm` — the whole point of both modules — reported success from
    /// both and produced a trace of ONE event with ZERO steps. Nothing errored. The
    /// artifact carried `debug_symbols`, so a test asserting "the artifact has debug
    /// info" was green over an artifact that could not be stepped. What was missing was
    /// the source-level instrumenter, which only runs on the debugging compile path.
    ///
    /// So the assertion is on `variables` / `functions`, which the instrumenter fills
    /// and which an ordinary compile leaves empty — and it is a PAIR, because either
    /// half alone is satisfiable by a build that always instruments or never does.
    /// With the instrumentation restored the same program traces to 27 events, 8 steps
    /// and 3 calls.
    #[test]
    fn the_debug_mode_instruments_the_program_and_the_program_mode_does_not() {
        let files = tree(&[
        ...
        let plan = resolve_vfs(&files, "proj").expect("resolves");

        let debugging = match compile_resolved(&plan, &files, false, true) {
            Ok((CompiledFromVfs::Program(program), _)) => program,
            other => panic!("the debugging compile must produce a program: {:?}", other.is_ok()),
        };
        let ordinary = match compile_resolved(&plan, &files, false, false) {
            Ok((CompiledFromVfs::Program(program), _)) => program,
            other => panic!("the ordinary compile must produce a program: {:?}", other.is_ok()),
        };

        let debug_vars: usize =
            debugging.debug_symbols.debug_infos.iter().map(|d| d.variables.len()).sum();
        let debug_fns: usize =
            debugging.debug_symbols.debug_infos.iter().map(|d| d.functions.len()).sum();
        let plain_vars: usize =
            ordinary.debug_symbols.debug_infos.iter().map(|d| d.variables.len()).sum();
        let plain_fns: usize =
            ordinary.debug_symbols.debug_infos.iter().map(|d| d.functions.len()).sum();

        assert!(
            debug_vars > 0 && debug_fns > 0,
            "the debugging compile records source-level variables and functions; \
             got {debug_vars} variables and {debug_fns} functions. Without them a tracer \
             consuming this artifact records no steps."
        );
        assert_eq!(
            (plain_vars, plain_fns),
            (0, 0),
            "the ordinary compile must NOT instrument: it is a different program, and a \
             caller asking for a circuit must not silently get an instrumented one"
        );

        // `variables` alone is NOT enough, and the mutation harness proved it: making the
        // instrumenter run unconditionally left this test green, because the debug TABLES
        // are emitted under `CompileOptions::instrument_debug` while the AST rewrite and
        // the `__debug` crate link are separate switches. An ordinary compile that had
        // silently been instrumented would therefore ship a different circuit and still
        // report zero variables.
        //
        // `file_map` is the discriminator that sees it: `link_to_debug_crate` puts
        // `__debug/lib.nr` in the program, so its presence is a direct observation of
        // "this program was built for debugging" independent of which tables were filled.
        let paths = |artifact: &noirc_artifacts::program::ProgramArtifact| -> Vec<String> {
            artifact.file_map.values().map(|f| f.path.display().to_string()).collect()
        };
        let debugging_paths = paths(&debugging);
        assert!(
            debugging_paths.iter().any(|p| p.contains("__debug")),
            "the debugging compile links the __debug crate; file_map was {debugging_paths:?}"
        );
        assert!(
            debugging_paths.iter().any(|p| p == "proj/src/main.nr"),
            "and it still names the caller's own VFS path, which is this module's whole \
             point; file_map was {debugging_paths:?}"
        );
        let ordinary_paths = paths(&ordinary);
        assert!(
            !ordinary_paths.iter().any(|p| p.contains("__debug")),
            "the ordinary compile must not link the __debug crate; file_map was \
             {ordinary_paths:?}"
        );

        // The two artifacts are genuinely different programs, which is why `debug` is a
        // mode rather than a flag a caller can forget.
        assert_ne!(
            debugging.hash, ordinary.hash,
            "instrumentation changes the program, so the hashes must differ"
        );
    }
    ...
    fn a_clean_tree_reports_no_diagnostics() {
        let files = three_file_tree_with_dep();
        let plan = resolve_vfs(&files, "app").expect("resolves");
        let (_, warnings) = compile_resolved(&plan, &files, false, false).expect("compiles");
    ...
    fn a_transitive_local_dependency_resolves() {
        ...
        assert!(
            compile_resolved(&plan, &files, false, false).is_ok(),
            "the transitive tree compiles"
        );
    }
